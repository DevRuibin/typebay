...
function revert(): void {
  ...
  qs(".pageTest .ssWatermark")?.setText("typebay.projectbay.uk"); // Reset watermark text
...
async function generateCanvas(): Promise<HTMLCanvasElement | null> {
  ...
  const ssWatermark = [format(dateNow, "dd MMM yyyy HH:mm"), "typebay.projectbay.uk"];
...
export async function download(): Promise<void> {
  try {
    ...
    const filename = `typebay-result-${timestamp}.png`;
