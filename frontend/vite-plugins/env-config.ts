    ...
    load(id) {
      if (id === resolvedVirtualModuleId) {
        ...
        const prodConfig: EnvConfig = {
          isDevelopment: false,
          // empty string is allowed on purpose: it makes API calls relative to
          // the site origin (TypeBay serves its own static host)
          backendUrl: options.env["BACKEND_URL"] ?? "https://api.monkeytype.com",
